...
cmd:option('-exploration_rate', 0.2)
cmd:option('-episodes', 5, 'number of episodes')  -- 5: 58%, 10 not better
cmd:option('-lamda', 0.3)

-- training options
cmd:option('-batch_size', 100, 'batch size')
cmd:option('-nepochs', 30, 'number of epochs')
cmd:option('-learning_rate',5e-3,'learning rate')
cmd:option('-lr_decay_every', 3)
...
--  local l_all = label.new():resize(nEpisodes, N, T, 2)
  ...
  return lossSum/nEpisodes, grads
end
